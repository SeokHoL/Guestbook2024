...
public class PageResultDTO<DTO,EN> {
    ...
    public PageResultDTO(Page<EN> result, Function<EN, DTO> fn) {
        //매개변수로 전달 받은 결과행들과 Entity를 DTO를 변환한 fn을 사용해서 GuestbookDTO객체를 저장한 리스트
    ...
    private  void makePageList(Pageable pageable){
        ...
        //현재 화면에 보여질 임시 마지막 페이지 변화
        int temEnd =(int) (Math.ceil(page/10.0)) * 10;
        start = temEnd - 9;
        
        //삼항조건연산자에서 조건식이 true면 마지막 화면이 아닌경우, false면 마지막화면이라는 의미
        //전체페이지번호가 31일때: 마지막화면이 아닌경우 1~3번째 화면(10,20,30), 마지막 화면은 4번째 화면을 의미(31).
        end = totalPage > temEnd ? temEnd:totalPage;
        pageList  = IntStream.rangeClosed(start,end).boxed().collect(Collectors.toList());
        prev  = start > 1;  //2번째~마지막화면까지 true
        next = totalPage > temEnd;  //1번째부터 ~마지막바로전 화면까지 true
